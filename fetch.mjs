...
// نص عربي إن وُجد بعد الشرطة، وإلا النص كما هو
const arabicPart = s => {
...
if (currentEvent) {
  news.push({
    ...
    body: `تصدّر دورينا «${league.standings.results[0]?.entry_name}» بـ ${league.standings.results[0]?.total} نقطة.`
  });
...
const data = {
  ...
  newEntries: (league.new_entries?.results || []).map(r => ({
    team: r.entry_name,
    manager: `${r.player_first_name} ${r.player_last_name}`.trim()
  })),
